...
#pragma warning restore 8618
    ...
    public override bool IsInDataSection(ulong virtualAddress)
    {
        var rva = GetRva(virtualAddress);
        var section = peSectionHeaders.FirstOrDefault(s => rva >= s.VirtualAddress && rva < s.VirtualAddress + s.VirtualSize);
        if (section == null)
            return false;

        return section.IsData;
    }
}
